...
class Vt_ProductsConfig_Block_List extends Mage_Catalog_Block_Product_List
{
	...
	protected function _beforeToHtml(){
		$pagesize=Mage::app()->getRequest()->getParam('pagesize');
		 ...
		 $new    = Mage::app()->getRequest()->getParam('new');
		 if($new!='') $this->setTemplate('vt/productsconfig/newproduct.phtml');
		if ($catid != '') {
		...
		if (is_null($this->_productCollection)) {
			...
			switch ($product_sort_by){
				...
				case 'random':
					...
					break;
				case 'top_rating':
					$collection->getSelect()->order('vt_rating_summary desc');
					break;
				case 'most_reviewed':
					$collection->getSelect()->order('vt_review_count desc');
					break;
				case 'most_viewed':
					$collection->getSelect()->order('vt_viewed_count desc');
					break;
				case 'best_sales':
					$collection->getSelect()->order('vt_ordered_count desc');
					break;
			}
