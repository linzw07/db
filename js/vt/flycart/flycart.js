...
function FlycartCreate(){
	if(typeof(defaultConfig) == 'object'){
		...
		//var checkPage = $$('.product-view .flycart-qty-btn'); && checkPage == ''
		if (pageView != '' && flagcart==0) {			
			ajaxcartConfig = new FlycartConfigClass(defaultConfig);
			flagcart = 1;
		}		
	}
...
FlycartConfigClass.prototype = {
	initialize: function(config){		
		...
		if($$('div.category-products').length > 0){
			...
			for(var j=0;j<$$('div.category-products').length;j++){
				var elements = $$('div.category-products')[j].getElementsByClassName('btn-cart');
